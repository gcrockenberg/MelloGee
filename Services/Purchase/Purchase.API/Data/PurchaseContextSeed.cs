namespace Me.Services.Purchase.API.Data;

using Me.Services.Purchase.Domain.AggregatesModel.BuyerAggregate;

public class PurchaseContextSeed
{
    public async Task SeedAsync(PurchaseContext context, IWebHostEnvironment env, IOptions<PurchaseSettings> settings, ILogger<PurchaseContextSeed> logger)
    {
        var policy = CreatePolicy(logger, nameof(PurchaseContextSeed));
    ...
    private IEnumerable<CardType> GetCardTypesFromFile(string contentRootPath, ILogger<PurchaseContextSeed> log)
    {
    ...
    private IEnumerable<OrderStatus> GetOrderStatusFromFile(string contentRootPath, ILogger<PurchaseContextSeed> log)
    {
    ...
    private AsyncRetryPolicy CreatePolicy(ILogger<PurchaseContextSeed> logger, string prefix, int retries = 3)
    {
